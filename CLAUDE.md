...
- **Known open problems** (see session handoff for the latest — may be stale by the time you read this):
  ...
  - **Issue D (P0) — `asset_signal_history` disk growth** (discovered 2026-05-07): `_append_history` in `signal_service.py` writes on lines 622, 641, 660, 709 unconditionally every sweep — not only on label transitions. DB is 3.3 GB total; `asset_signal_history` alone is 2.6 GB (79%). Growth rate ~174 MB/day (~387k rows/day). Headroom to full: TBD — confirm Railway Postgres volume limit in dashboard. **Fix path (order is mandatory): (1) transition-only write first** — make `_append_history` gate on `prev_label != signal.label`; **(2) retention prune second** — DELETE rows older than N days. Do NOT prune first: it only buys ~10 days and the problem recurs. `observation_match_logs` (127 MB, no purge) is a secondary concern — growth currently paused while eBay ingest is dark, but needs a purge policy before Browse API is wired.
