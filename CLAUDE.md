...
- **Known open problems** (see session handoff for the latest — may be stale by the time you read this):
  ...
  - **Issue E — CLOSED AS BY DESIGN** (diagnosed 2026-05-07): Original hypothesis (current_n threshold mismatch) was wrong. Audit confirmed: 98.6% of INSUFFICIENT_DATA assets (2,353/2,390) have `downgrade_reason='bulk_baseline_price'` — intentional downgrade for bulk commons where absolute prices are too low for reliable percentage-change signals. `current_n=10` for virtually all assets; `MIN_CURRENT_N_FOR_SIGNAL=3` is correct and not the problem. **Do not touch `MIN_CURRENT_N_FOR_SIGNAL`.** Open product question: is the 58% bulk-commons rate acceptable, or is `bulk_baseline_price` floor too aggressive? Calibration decision requiring user feedback — see `_apply_signal_downgrade` in signal_service.py.
