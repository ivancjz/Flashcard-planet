...
### Lesson 6: Catalog source and price source must not be the same thing

`fetch_set_entries` was designed to simultaneously serve as catalog source (which cards exist) and price source (what they sell for). This works for sets with full YGOPRODeck price coverage (POTE 2022, TOCH 2020) but silently produces zero assets for sets where YGOPRODeck has card metadata but no price data (LEDE 2024, PHNI 2024, AGOV 2023, and likely the 8 sets added in PR #28). The `scheduler_run_log` shows `status=success, sets_failed=[]` for all 50+ runs — the ingest is "working" in every observable sense while systematically omitting 11 of 13 configured sets.

**Result**: any price-source coupling in a catalog-building function creates a silent data gap that is invisible to all existing monitoring (no error logs, no failed sets, no Discord alerts). For any new game's ingest pipeline, the catalog stage must be separated from the price stage. The planned Phase B fix: replace `fetch_set_entries` (which filters price=0) with a catalog-only `fetch_set_cards` that builds all assets first, then let eBay be the price source. Track this in the Phase B PR description.
