...
- **Known open problems** (see session handoff for the latest — may be stale by the time you read this):
  ...
  - **Backup system live.** GitHub Actions `daily-backup.yml` runs at 04:00 UTC daily → `backup.sql.gz` asset on `ivancjz/flashcard-planet-backups` GitHub Releases. 30-day rolling retention. Discord alert on failure. APScheduler watchdog (`backup-freshness-check`, runs ~06:00 UTC after each deploy via 7800s startup delay) verifies a fresh backup appeared and alerts if not. Quarterly local download via `backend/scripts/backup_to_local.sh`. Restore: `docs/runbooks/restore-from-backup.md`. **First restore drill: not yet executed** (D4 of PR #14a will fill this in).
