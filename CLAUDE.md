...
- **Known open problems** (see session handoff for the latest — may be stale by the time you read this):
  ...
  - **Issue E (P1, diagnosis pending) — `MIN_CURRENT_N_FOR_SIGNAL` may exceed Pokemon TCG API update frequency** (discovered 2026-05-07): 113/200 least-varying Pokemon assets have ≥5% price range over 14 days (some +950%), yet 97.7% of all assets are INSUFFICIENT_DATA, dominantly `has_baseline_no_current`. Hypothesis: Pokemon TCG API updates prices infrequently; `current_n` (count of price points in recent window) falls below `MIN_CURRENT_N_FOR_SIGNAL` not because data is absent but because repeated polls return identical values. **Do NOT touch threshold until confirmed:** run distribution of `current_n` from `signal_context` across pokemon assets; cross-check: assets with ≥2 distinct prices in 14d — what is their `current_n`? If `current_n < MIN_CURRENT_N_FOR_SIGNAL`, hypothesis confirmed. Fix path (if confirmed): lower threshold to match API update frequency, OR increase ingest frequency. **This is a different system from Issue D** — do not conflate, separate fix.
  - **Issue D (P0) — `asset_signal_history` disk growth** (discovered 2026-05-07): `_append_history` in `signal_service.py` (lines 622, 641, 660, 709) writes one row per asset per sweep unconditionally — not only on label transitions. DB is 3.3 GB total; `asset_signal_history` alone is 2.6 GB (79%), 5.8 M rows. Growth rate ~174 MB/day (~387k rows/day at 96 sweeps/day × 4,033 assets). Headroom to full: TBD — confirm Railway Postgres volume limit in Railway dashboard → Postgres → Storage tab. **Audit findings (2026-05-07):** All 4 callsites are transition-safe — all write INSUFFICIENT_DATA or a classified label for every asset every sweep; a transition guard is semantically correct for all four. All 5 product queries (`/cards?sort=recent`, watchlist export, batch cards sort=recent, card detail signal history, `/alerts`) already filter `WHERE previous_label IS NOT NULL AND label IS DISTINCT FROM previous_label` — none depend on every-sweep rows. `get_daily_snapshot_signals` (signal_service.py:753) is confirmed dead code: its only caller (`site.py:signals_page`) was removed in the SPA migration commit 2b84b10; no production path calls it. Dead-code confirmation clears the one audit blocker. **Fix path (order is mandatory): (1) transition guard first** — single guard at top of per-asset loop: `if prev_label is None or prev_label != signal.label.value: _append_history(...)`. First-time writes (prev_label=None) are kept; repeats are dropped. Do NOT add per-callsite logic — one guard covers all 4. **(2) retention prune second** — DELETE rows older than N days after guard is deployed and inflow confirmed reduced. Do NOT prune first: it only buys ~10 days and the problem recurs. **48h verification gates (all must hold after deploy):** rows_written/day drops from ~387k toward transition rate; ratio rows_written:transitions approaches 1:1 via `/admin/diag/signal-history-stats`; DB total size growth rate drops from ~174 MB/day to <10 MB/day. `observation_match_logs` (127 MB, no purge) is a secondary concern — growth currently paused while eBay ingest is dark, but needs a purge policy before Browse API is wired.
