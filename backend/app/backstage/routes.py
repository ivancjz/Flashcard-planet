...
@router.get("/diag/prb-post-sweep")
def admin_prb_post_sweep(
    _: None = Depends(require_admin_key),
    db: Session = Depends(get_database),
):
    """Post-PR-B sweep observation: signal distribution, insufficiency breakdown, recent sweep sample, sweep cadence check."""
    # SQL 1: current signal distribution (asset_signals is one-row-per-asset upsert)
    ...
    # SQL 2: insufficiency reason breakdown (status/reason lives in signal_context JSONB)
    insuff = db.execute(text("""
        SELECT
    ...
    # SQL 3: 20 assets with largest |price_delta_pct| from the most recent sweep window
    # computed_at is the correct column (not created_at); baseline_n/current_n are in signal_context
    recent_sweep_time = db.execute(text(
        "SELECT MAX(computed_at) FROM asset_signals"
    )).scalar()
    sample = db.execute(text("""
        SELECT
            ...
            (s.signal_context->>'baseline_n')::int AS baseline_n,
            (s.signal_context->>'current_n')::int  AS current_n,
            s.price_delta_pct::float,
            s.computed_at::text
        FROM asset_signals s
        JOIN assets a ON a.id = s.asset_id
        WHERE s.computed_at > NOW() - INTERVAL '20 minutes'
        ORDER BY ABS(COALESCE(s.price_delta_pct, 0)) DESC
        LIMIT 20
    """)).fetchall()

    # SQL 4: confirm sweep ran post-deploy (job_name is 'signals', not 'signal-sweep')
    sweep_runs = db.execute(text("""
    ...
    return {
        "sql1_distribution": [{"label": r[0], "n": r[1], "pct": round(100*r[1]/total, 1) if total else 0} for r in dist],
        "sql1_total": total,
        "sql2_insufficiency_reasons": [{"reason": r[0], "downgrade_reason": r[1], "n": r[2]} for r in insuff],
        "sql3_most_recent_computed_at": str(recent_sweep_time),
        "sql3_sample_in_window": len(sample),
        "sql3_top_delta": [
            {"name": r[0], "set": r[1], "label": r[2], "reason": r[3], "downgrade": r[4],
             "baseline_n": r[5], "current_n": r[6], "delta_pct": r[7], "computed_at": r[8]}
            for r in sample
        ],
        "sql4_recent_sweep_runs": [
            {"status": r[1], "started_at": r[2], "duration_s": r[4], "records": r[5]}
            for r in sweep_runs
        ],
        "sql4_sweep_confirmed": len(sweep_runs) > 0 and sweep_runs[0][1] == "success",
    }
