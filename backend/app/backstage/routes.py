...
# REMOVE AFTER: market_events correction run confirmed (one-shot, 2026-05-19)
@router.post("/trigger/fix-market-events-2026-05-19")
def fix_market_events_2026_05_19(
    _: None = Depends(require_admin_key),
    db: Session = Depends(get_database),
) -> dict:
    """Apply verified corrections to 5 INFLUENCER market_events rows + 1 INSERT.

    Based on research 2026-05-19. Run once, then remove this endpoint.
    """
    results = {}

    stmts = [
        ("logan_paul_box_break", text("""
        ...
        """)),
        ("chaos_rising_announcement", text("""
            UPDATE market_events SET
    ...
    for key, stmt in stmts:
        result = db.execute(stmt)
        results[key] = result.rowcount

    db.commit()

    # Verification: return all INFLUENCER rows + total count
    all_rows = db.execute(
        text("SELECT event_type, event_date::date, description, source_url FROM market_events ORDER BY event_date")
    ).fetchall()
    results["total_events"] = len(all_rows)
    results["influencer_events"] = [
        {"date": str(r[1]), "description": r[2][:80], "source": r[3]}
        for r in all_rows if r[0] == "INFLUENCER"
    ]
