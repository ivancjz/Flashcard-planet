(function () {
  ...
  function renderSnapshot(snapshot) {
    ...
    provider.innerHTML = `
      ...
      <div class="metric-stack">
        ${metricCard("追踪卡牌", "Tracked assets", ps.tracked_assets, `${ps.real_history_assets} 条真实历史`, `${ps.real_history_assets} with real history`)}
        ${metricCard("近24小时新增", "Recent rows (24h)", ps.recent_real_rows_24h, `${ps.assets_changed_24h} 张价格变动`, `${ps.assets_changed_24h} assets changed`)}
        ${metricCard("行变化率(24h)", "Row change rate", ps.row_change_pct_24h, `7天: ${ps.row_change_pct_7d}`, `7d: ${ps.row_change_pct_7d}`)}
        ${metricCard("已配置数据源", "Configured providers", ps.configured_provider_count, `当前来源: ${ps.active_source}`, `Active: ${ps.active_source}`)}
      </div>`;

    const ss = snapshot.signal_snapshot;
    signalOps.innerHTML = `
      <div class="module-head">
        <p class="card-kicker">${t("关注列表 / 预警 / 诊断", "Watchlists / Alerts / Diagnostics")}</p>
        <h2>${t("运营循环", "Operator Loop")}</h2>
      </div>
      <div class="metric-stack">
    ...
    topValue.innerHTML = `
      <div class="module-head">
        <p class="card-kicker">${t("最高价值", "Top Value")}</p>
        <h2>${t("当前最高价格卡牌", "Highest Current Prices")}</h2>
      </div>
      <div class="list-shell">
        ${snapshot.top_value.map((item) => listItem(item.name, [
          item.latest_price,
          item.set_name || t("系列未知", "Set name unavailable"),
    ...
    topMovers.innerHTML = `
      <div class="module-head">
        <p class="card-kicker">${t("涨跌榜", "Top Movers")}</p>
        <h2>${t("近期最大价格变动", "Largest Recent Step Moves")}</h2>
      </div>
      <div class="list-shell">
        ${snapshot.top_movers.map((item) => listItem(item.name, [
          item.latest_price,
          `${t("变动", "Move")}: ${item.absolute_change}`,
    ...
    highActivity.querySelector(".module-head").innerHTML = `
      <p class="card-kicker">${t("高活跃度 v2 对比基准", "High-Activity v2 vs Baseline")}</p>
      <h2>${ha.headline}</h2>`;
    ...
    poolGrid.innerHTML = snapshot.pools.map((pool) => `
      <article class="pool-card">
        ...
        <span>${t("历史覆盖率", "History coverage")}: ${pool.assets_with_history}</span>
        ...
        <span>${t("无变动卡牌", "No movement")}: ${pool.no_movement_assets}</span>
      </article>`).join("");
  ...
  function renderLookupResults(prices, predictions, history) {
    ...
    lookupResults.innerHTML = prices.slice(0, 4).map((item) => {
      ...
      return `<article class="result-card">
        ...
        <span class="result-meta">${t("预测", "Prediction")}: ${pred?.prediction || t("暂无", "Unavailable")}</span>
    ...
    if (!history) {
      lookupHistory.innerHTML = `<article class="history-card">
        <strong>${t("暂无历史数据", "No history returned")}</strong>
        <span class="history-meta">${t("换一张已追踪卡牌试试。", "Try another tracked asset.")}</span>
      </article>`;
      return;
    }
    ...
    lookupHistory.innerHTML = `<article class="history-card">
      ...
      <span class="history-meta">${t("当前价格", "Current price")}: ${history.current_price} ${history.currency}</span>
  ...
  async function runLookup(rawQuery) {
    ...
    if (!query) {
      lookupStatus.textContent = t("请先输入卡牌名称。", "Enter a tracked asset name first.");
      return;
    }
    lookupStatus.textContent = t(`查询中 "${query}"...`, `Looking up "${query}"...`);
    try {
      ...
      lookupStatus.textContent = t(`"${query}" 实时查询结果`, `Live results for "${query}"`);
    } catch (error) {
